...
- For a known Session ID, use `arkcli agent session compact <session-id> [--instructions <text|@file>] [--session-thread-id <id>]`. It preflights an idle Session, sends the frontend-compatible slash envelope, and waits after the final sent event cursor. Normal `session.thread_status_idle` and `session.status_idle` events with `stop_reason=end_turn` complete the manual command. `agent.thread_context_compacted` is optional confirmation that compaction actually occurred and may be absent when the backend has no compactable messages. Failure/termination events and timeouts return nonzero.
- `session compact` waits through SSE for 120 seconds by default; `--poll` uses event-list polling, and `--raw`/`--format jsonl` preserves raw events. `--dry-run` is a zero-network Workflow Client Preview of idle preflight, escaped payload, cursor wait, the two-idle success predicate, and the optional compacted confirmation event.
- REPL `/compact <instructions>` XML-escapes and forwards all instructions in `<command-args>`; `/clear` remains unchanged. Send acceptance alone does not complete the command, and idle-only completion must not be reported as confirmed history compaction.
...
- `+new session` is the Managed Agent session entry point (early PRD usage of `+chat` is understood as this command):
  ...
  - `/compact` and `/clear` send `user.message` via slash envelope; do not judge backend completion based solely on successful transmission. Manual `/compact` completes on normal thread idle plus session idle. `agent.thread_context_compacted` is stronger confirmation emitted only when the service actually compacts and may be absent when there are no compactable messages. `/clear` follows the service-side event response. Both continue the REPL after completion.
