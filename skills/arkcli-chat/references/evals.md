...
| Capability notices | Explicit older versioned model ID; unrelated metadata without reasoning declarations; English locale. | Preserve the exact execution ID; warn only from present metadata; emit no false reasoning warning; keep warnings on stderr and stdout unchanged. |
